...
export default function StageContainer({ stage, allStages }: Props) {
  ...
  return (
    <div className={styles.container}>
      <div className={`${styles.streamContainer} ${stage.isLive ? '' : styles.streamYt}`}>
        {loginStatus === 'loggedIn' ? (
            ...
            <div className={cn(styles.stream, styleUtils.appear, styleUtils['appear-first'])}>
              {updatedStage.stream ? (
                <iframe
                  ...
                  allowFullScreen
                  frameBorder="0"
                  src={`${updatedStage.stream}${updatedStage.stream.includes('?') ? '&' : '?'}autoplay=1&mute=1`}
                ...
                >
                  <div className={cn(styles.messageContainer, styles.messageContainerWrap)}>
                    <h2 className={styles.stageName}>{updatedStage.name}</h2>
                    <p>Live stream will appear here when the stage is active.</p>
                  </div>
                </div>
              )}
          ...
          <div className={cn(styles.stream, styleUtils.appear, styleUtils['appear-first'])}>
            <div className={cn(styles.messageContainer, styles.messageContainerWrap)}>
              <h2 className={styles.stageName}>Sign in to enter the live stage</h2>
              <p>Your TechVerse session is required before you can join a stage.</p>
              <Link
                href={`/?login=1&returnTo=${encodeURIComponent(`/stage/${stage.slug}`)}`}
                className={styles.button}
              >
                Login / Register
              </Link>
      ...
      {stage.isLive ? null : <ScheduleSidebar allStages={updatedStages} />}
    </div>
  );
}
